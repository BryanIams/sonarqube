...
public class GroupsAction implements PermissionsWsAction {
  ...
  public void handle(Request request, Response response) throws Exception {
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      int total = dbClient.groupPermissionDao().countGroupsByPermissionQuery(dbSession, org.getUuid(), query);
  ...
  private List<GroupDto> findGroups(DbSession dbSession, OrganizationDto org, PermissionQuery dbQuery) {
    List<Long> orderedIds = dbClient.groupPermissionDao().selectGroupIdsByQuery(dbSession, org.getUuid(), dbQuery);
    List<GroupDto> groups = dbClient.groupDao().selectByIds(dbSession, orderedIds);
    if (orderedIds.contains(0L)) {
      groups.add(0, new GroupDto().setId(0L).setName(DefaultGroups.ANYONE));
    }
    return Ordering.explicit(orderedIds).onResultOf(GroupDto::getId).immutableSortedCopy(groups);
  }
